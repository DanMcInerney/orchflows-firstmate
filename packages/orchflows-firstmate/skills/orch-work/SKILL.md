...
Put the assignment, intended workspace and input state, resolved guidance paths and scoped caller choices in the brief; instruct the worker to read and apply the Make sections and to commit before reporting. Record the task ID and its role in the backlog item note. FirstMate's watcher, steering, recovery and delivery apply unchanged; the [FirstMate mapping](../../docs/firstmate.md) records the exact commands and checkpoints.
