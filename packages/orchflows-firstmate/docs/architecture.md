...
**The client supports legacy single read-only Work/Review and an explicitly admitted bounded Linux dynamic profile.** The [client contract](firstmate-client.md) requires the actual matching FirstMate controller, current normal root scout generation and exact retained fork snapshot. Every dispatch validates these inputs. Dynamic additionally requires the controller's dynamic and workflow-review capabilities plus the exact dynamic attachment; no prompt or request body can upgrade a legacy attachment.

Dynamic composes up to 32 FirstMate-owned Work/Review components. Each new request freezes the clean root worktree commit, writer results retain input_commit/output_commit, and the root joins useful commits with ordinary Git before requesting one fresh independent Review. Gather all prior work, check and commit the candidate, gather Review, then make one repair/check pass without another Review. FirstMate owns worktree allocation, worker settings, communication, recovery, cancellation and root delivery; the package adds no scheduler or writer mechanism.

Standalone Review remains an explicitly authorized audit of the attachment's original clean commit. Legacy Work remains one read-only request. A selected complete custom library may compose the same admitted primitives. Nesting, component continuation, model/effort overrides, remote homes, ship/no-mistakes delivery, Build, SelfImprove and design-loop remain gated. Stop before unsupported work, delegation, repairs or trials. Package setup, resolve, doctor and read-only transcript inspection remain available; package readiness does not establish operational fleet integration.
