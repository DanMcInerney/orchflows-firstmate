...
A standalone Orchflows variant for **FirstMate running in Herdr**, with Claude Code and Codex CLI as worker harnesses. Development version `0.1.0-dev.9` supplies Work/Review assignment controls, a FirstMate-owned saved-workflow home, and selected composition scopes. **Execution requires the matching controller and an exact retained task attachment.** Linux is the current development platform.

Select a saved workflow by `library:skill` during ordinary FirstMate brief intake. FirstMate retains its complete libraries, resolves assignment settings through its harness owner, and launches Work/Review components through its existing worktree and lifecycle systems. Each selected dynamic invocation has one independent Review and one repair/check pass. A composing Work trial can delegate only its explicitly authorized calls; the full group remains bounded to 32 components.

Author libraries in the assigned worktree, trial and review their actual output, then use ordinary ship/local-only delivery and FirstMate's publication operation. Published changes affect future runs; active tasks and replacements keep their retained snapshots. See the [client and workflow-intake contract](docs/firstmate-client.md).

Legacy single read-only Work/Review and existing bounded dynamic attachments keep their contracts. Native Windows, component continuation, general SelfImprove, other ship modes and full optional-library parity remain unverified or gated. Source, fixture checks and actual workers are separate evidence; package setup does not certify a running fleet.

The source remains pinned to Orchflows `ca72258493480ddcfe73b3f01d0475ad532e4726`; all upstream paths and optional-library assets remain retained. The experimental design-loop example stays inactive migration source and is not used for this work.
