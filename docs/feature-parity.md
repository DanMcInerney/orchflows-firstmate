...
**Current implementation:** dev.9 adds assignment controls, FirstMate-owned library publication/discovery, composition-scoped Review and a bounded descendant seam. The [implementation contract](plug-and-play-implementation.md) describes those changes; [verification](plug-and-play-verification.md) separates source, owner fixtures and actual workers. The historical increments below retain their original scope and evidence.
...
| Review | Fresh independent reviewer, scoped findings, no repairs | Separate component with reviewed-state identity and explicit review authorization | Legacy explicit audits preserved; each authorized dynamic invocation admits one fresh read-only Review after its prior results are gathered, freezes the joined caller commit and fences a second Review within that call. [Current evidence](dynamic-verification.md) |
...
| Build workflow | Create workflows/guidance/libraries, trial them, review and refine | Author in assigned workspace; collect package artifact and publish/install through the selected authorized delivery | Complete leaf/composing library authoring and scoped Work trials through selected dynamic calls; FirstMate-owned publication after ordinary delivery. One level of authorized descendants is implemented; full Build parity and portability remain unverified |
...
| Model and effort | Per-setting caller precedence, assignment overrides, repair settings, unsupported-value errors | FirstMate dispatch uses effective worker controls; continue or replace by explicit capability | Negotiated independent model/effort precedence, accepted profile/replay identity and FirstMate flag validation implemented; actual model availability and native outcomes remain separate evidence |
...
| Home setup | Complete package, runtime, editable libraries, recovery on failed swap | Own home/package/catalog identity; host configuration edits become explicit | Package setup preserved; FirstMate-owned editable home, committed-library publication, brief identity selection and future-bundle refresh added in dev.9 |
...
| Host manifests and worker discovery | Claude and Codex can discover the distribution | Independent namespace and FirstMate launch catalog; never substitute normal Orchflows | Project-enabled launch supplies retained core/custom skill paths and client context. Current worker evidence is in [dynamic verification](dynamic-verification.md); no global user installation |
| Updates | Preserve user libraries and recover from a failed core swap | Retain complete task-pinned dependency sets across active tasks; activate new defaults separately | Existing package swap retained; one complete immutable task snapshot implemented; project enablement also retains immutable default bundles; pruning and general rollback remain planned |
